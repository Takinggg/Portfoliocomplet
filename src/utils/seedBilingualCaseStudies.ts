 ...
 * Note: Input type is intentionally loose due to complex bilingual structure
 */
export function convertBilingualToCaseStudy(bilingualCs: Record<string, any>): CaseStudy {
  return {
    id: bilingualCs.id,
    ...
    challenge: {
      ...
      painPoints: Array.isArray(bilingualCs.challenge.painPoints) 
        ? (typeof bilingualCs.challenge.painPoints[0] === 'object' 
          ? bilingualCs.challenge.painPoints.map((p) => p.fr) 
    ...
    solution: {
      ...
      approach: Array.isArray(bilingualCs.solution.approach)
        ? (typeof bilingualCs.solution.approach[0] === 'object'
          ? bilingualCs.solution.approach.map((a) => a.fr)
    ...
    results: {
      ...
      metrics: bilingualCs.results.metrics.map((m) => ({
    ...
    process: bilingualCs.process.map((p) => ({
      phase: p.phase,
