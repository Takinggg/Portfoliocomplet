  ...
  return (
    <div 
      ...
      style={{ 
        ...
        backgroundColor: hasLanguagePrefix ? 'rgba(204, 255, 0, 0.1)' : 'rgba(255, 87, 87, 0.1)',
        border: hasLanguagePrefix ? '1px solid rgba(204, 255, 0, 0.3)' : '1px solid rgba(255, 87, 87, 0.3)',
      ...
      <div className="p-4 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm mb-1" style={{ color: hasLanguagePrefix ? '#CCFF00' : '#FF5757' }}>
              {hasLanguagePrefix ? 'âœ… URLs bilingues actives' : 'âš ï¸ Ancienne URL dÃ©tectÃ©e'}
            </h3>
            
            <p className="text-xs text-[#F4F4F4]/70 mb-2">
              URL actuelle: <code className="text-[#CCFF00]">{location.pathname}</code>
            </p>
            
            {hasLanguagePrefix ? (
              <p className="text-xs text-[#F4F4F4]/60">
                Les URLs bilingues fonctionnent correctement ! ðŸŽ‰
              </p>
            ) : (
