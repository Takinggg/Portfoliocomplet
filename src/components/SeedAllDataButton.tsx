 ...
 * Bouton pour initialiser TOUTES les données dans Supabase
 */
...
export function SeedAllDataButton() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      if (result.success > 0) {
        // Reload après 3 secondes pour voir les nouvelles données
        setTimeout(() => {
  ...
  return (
    <Card className="bg-gradient-to-br from-[#00FFC2]/10 to-[#00FFC2]/5 border-[#00FFC2]/20 p-6">
      <div className="flex items-start gap-4">
        <div className="p-3 rounded-lg bg-[#00FFC2]/10">
          <Database className="h-8 w-8 text-[#00FFC2]" />
        </div>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-white/70 mb-4">
            Crée un portfolio complet et professionnel avec :
          </p>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4 text-sm text-white/60">
            ...
            <div className="flex items-center gap-2">
              <CheckCircle className="h-4 w-4 text-[#00FFC2]" />
              <span>5 Témoignages clients</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="h-4 w-4 text-[#00FFC2]" />
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 mb-4">
            <p className="text-xs text-yellow-200 flex items-start gap-2">
              ...
              <span>
                <strong>Important :</strong> Déployez d'abord le serveur complet
                ...
                {result.success > 0 ? (
                  ...
                  <>
                    ...
                    <span className="font-medium">
                      ❌ Échec - Vérifiez que le serveur complet est déployé
                    </span>
                  </>
                )}
              </div>
          ...
          {loading && (
            <div className="mt-4 p-3 bg-black/30 rounded-lg border border-white/10">
              <div className="flex items-center gap-2 text-xs text-white/60 mb-2">
                ...
                <span>Logs de création (voir la console)</span>
              </div>
              <div className="text-xs text-white/40 font-mono">
                Ouivrez la console (F12) pour suivre la progression en détail...
              </div>
            </div>
          )}
        </div>
