...
export function BlogTab({ onRefresh, loading = false }: BlogTabProps) {
  ...
  // Form state with enhanced multilingual support
  ...
  const addTag = (lang: 'fr' | 'en') => {
    ...
    if (tagInput.trim() && !currentTags.includes(tagInput.trim())) {
      setFormData({
        ...
        // Update legacy tags field with French tags for backward compatibility
        tags: lang === 'fr' ? [...currentTags, tagInput.trim()] : formData.tags,
      });
      setTagInput("");
    }
  };
  ...
  const openEditDialog = (post: BlogPost) => {
    setEditingPost(post);
    const postData = post as any; // Cast to access new bilingual fields
    
    setFormData({
      title_fr: postData.title_fr || post.title || "",
      title_en: postData.title_en || post.title || "",
      slug_fr: postData.slug_fr || post.slug || "",
      slug_en: postData.slug_en || post.slug || "",
      excerpt_fr: postData.excerpt_fr || post.excerpt || "",
      excerpt_en: postData.excerpt_en || post.excerpt || "",
      content_fr: postData.content_fr || post.content || "",
      content_en: postData.content_en || post.content || "",
      coverImage: post.coverImage || "",
      category_fr: postData.category_fr || "développement",
      category_en: postData.category_en || "development",
      tags_fr: postData.tags_fr || post.tags || [],
      tags_en: postData.tags_en || post.tags || [],
      seo_description_fr: postData.seo_description_fr || "",
      seo_description_en: postData.seo_description_en || "",
      seo_keywords_fr: postData.seo_keywords_fr || [],
      seo_keywords_en: postData.seo_keywords_en || [],
      status: post.status,
      // Legacy fields
      slug: post.slug,
