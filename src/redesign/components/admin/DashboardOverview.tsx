...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({ 
  ...
  const [searchQuery, setSearchQuery] = useState('');
    const [selectedIds, setSelectedIds] = useState<Array<Client["id"]>>([]);
  ...
  const handleSelectOne = (id: Client["id"]) => {
  ...
  const handleBulkDelete = () => {
      if (onDeleteClient && confirm(`Supprimer ${selectedIds.length} client(s) ?`)) {
          selectedIds.forEach(id => onDeleteClient(id));
          setSelectedIds([]);
      }
  };

  const handleDeleteOne = (id: Client["id"]) => {
      if (onDeleteClient && confirm('Supprimer ce client ?')) {
          onDeleteClient(id);
      }
      setActiveMenuId(null);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 font-sans" onClick={() => setActiveMenuId(null)}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
          <div>
                <h1 className="text-4xl font-display font-bold text-white mb-1">Annuaire clients</h1>
                <p className="text-neutral-500 text-sm">Centralisez vos clients, leads et partenaires.</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <StatCard icon={Users} label="Clients au total" value={clients.length} change="+12" trend="up" />
            <StatCard icon={User} label="Clients actifs" value={totalActive} change="+4" trend="up" />
            <StatCard icon={User} label="Clients inactifs" value={totalInactive} change="-2" trend="down" />
      ...
      <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
              ...
              <div className="flex bg-[#050505] p-1 rounded-lg border border-white/5">
                      ...
                      >
                                                    {tab === 'all' ? 'Tous' : tab === 'active' ? 'Actifs' : tab === 'inactive' ? 'Inactifs' : 'Leads'}
                      </button>
                  ))}
              </div>
              ...
              {selectedIds.length > 0 ? (
                   <div className="flex items-center gap-4 bg-red-500/10 px-4 py-2 rounded-lg border border-red-500/20 animate-in fade-in w-full md:w-auto justify-between">
                       <span className="text-sm text-red-500 font-bold">{selectedIds.length} sélectionné(s)</span>
                       <button onClick={handleBulkDelete} className="text-xs bg-red-500 text-white px-3 py-1.5 rounded font-bold hover:bg-red-600 transition-colors">Supprimer</button>
                   </div>
              ) : (
              ...
              <table className="w-full text-left border-collapse">
                  <thead className="bg-[#080808] text-xs uppercase text-neutral-500 font-mono tracking-wider border-b border-white/5">
                      <tr>
                          ...
                          <th className="p-6 font-medium">Nom du client</th>
                          <th className="p-6 font-medium">ID</th>
                          <th className="p-6 font-medium">Rôle</th>
                          <th className="p-6 font-medium">Département</th>
                          <th className="p-6 font-medium">Date d'entrée</th>
          ...
          <div className="p-4 border-t border-white/5 flex justify-between items-center bg-[#080808]">
              <div className="text-xs text-neutral-500 font-mono uppercase tracking-widest">
                  {`Affichage de ${filteredClients.length} / ${clients.length} fiches`}
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
              <div className="bg-[#0A0A0A] border border-white/10 rounded-2xl p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-300 relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-center mb-8">
                      <div>
                        <h2 className="text-2xl font-display font-bold text-white">{editId ? 'Modifier le client' : 'Ajouter un client'}</h2>
                        <p className="text-neutral-500 text-sm">Renseignez les informations ci-dessous.</p>
                      </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                      <div className="grid grid-cols-2 gap-5">
                        <div className="space-y-2">
                            <label className="text-xs font-mono text-neutral-500 uppercase">Nom complet</label>
                            <input 
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-mono text-neutral-500 uppercase">Adresse email</label>
                            <input 
                      ...
                      <div className="grid grid-cols-2 gap-5">
                        <div className="space-y-2">
                            <label className="text-xs font-mono text-neutral-500 uppercase">Rôle / Poste</label>
                            <input 
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-mono text-neutral-500 uppercase">Département</label>
                            <input 
                      ...
                      <div className="space-y-2">
                            <label className="text-xs font-mono text-neutral-500 uppercase">Statut du compte</label>
                            <select
                      ...
                      <button type="submit" className="w-full py-4 bg-primary text-black font-bold rounded-lg hover:bg-white transition-colors flex items-center justify-center gap-2 uppercase tracking-wide text-sm mt-4">
                          <Save size={18} /> {editId ? 'Mettre à jour' : 'Créer le client'}
                      </button>
                  </form>
              </div>
          </div>
