...
export const FinanceManager: React.FC<FinanceManagerProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
      e.preventDefault();
    const client = clients.find(c => String(c.id) === newQuote.clientId);
      if (client) {
          onAddQuote({
              clientId: client.id,
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
       <div className="flex justify-between items-center">
          <div>
                ...
                <p className="text-neutral-400">Pilotez vos devis et factures.</p>
          </div>
          <div className="flex bg-black border border-white/10 rounded-lg p-1">
                    <button onClick={() => setView('quotes')} className={`px-4 py-2 rounded-md text-sm font-bold uppercase transition-colors ${view === 'quotes' ? 'bg-white text-black' : 'text-neutral-500 hover:text-white'}`}>Devis</button>
                    <button onClick={() => setView('invoices')} className={`px-4 py-2 rounded-md text-sm font-bold uppercase transition-colors ${view === 'invoices' ? 'bg-white text-black' : 'text-neutral-500 hover:text-white'}`}>Factures</button>
          </div>
      </div>
                          ...
                          {quotes.map(quote => (
                              <tr key={quote.id} className="hover:bg-white/5 transition-colors">
                                  ...
                                  <td className="p-4 text-right pr-6 flex justify-end gap-2">
                                      <button onClick={() => onDownload && onDownload(quote.id)} className="p-2 hover:bg-white/10 rounded text-neutral-500 hover:text-white" title="Télécharger le PDF"><Download size={18}/></button>
                          ...
                          {invoices.map(inv => (
                              <tr key={inv.id} className="hover:bg-white/5 transition-colors">
                                  ...
                                  <td className="p-4">
                                      ...
                                      }`}>{INVOICE_STATUS_LABELS[inv.status]}</span>
                                  </td>
                                  <td className="p-4 text-right pr-6 flex justify-end gap-2">
                                       {inv.status !== 'paid' && (
                                       ...
                                       <button onClick={() => onDownload && onDownload(inv.id)} className="p-2 hover:bg-white/10 rounded text-neutral-500 hover:text-white" title="Télécharger le PDF"><FileText size={18}/></button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
              <div className="bg-[#111] border border-white/10 rounded-2xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
                  <div className="flex justify-between items-center mb-6">
                      <h2 className="text-xl font-bold text-white">Créer un devis</h2>
