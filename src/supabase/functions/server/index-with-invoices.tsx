...
console.log("🚀 Starting COMPLETE server with ALL routes (including quotes + invoices)...");
...
app.post("/make-server-04919ac5/quotes/:id/convert", requireAuth, async (c) => {
  try {
    ...
    if (quote.status !== "accepted") {
      return c.json({ success: false, error: "Only accepted quotes can be converted" }, 400);
    }

    if (quote.convertedToInvoice) {
      return c.json({ success: false, error: "Already converted" }, 400);
    }
...
app.get("/make-server-04919ac5/invoices", requireAuth, async (c) => {
  try {
    ...
    console.log(`✅ Found ${sortedInvoices.length} invoices`);
...
app.get("/make-server-04919ac5/invoices/:id", requireAuth, async (c) => {
  try {
    ...
    const kvKey = invoiceId.startsWith("invoice:") ? invoiceId : `invoice:${invoiceId}`;
    
    const invoice = await kv.get(kvKey);
...
app.put("/make-server-04919ac5/invoices/:id", requireAuth, async (c) => {
  try {
    ...
    const existingInvoice = await kv.get(kvKey);
    ...
    await kv.set(kvKey, updatedInvoice);
...
console.log("✅ ALL 4 INVOICES ROUTES ADDED!");
...
console.log("   INVOICES: /invoices (GET), /invoices/:id (GET/PUT/DELETE) ✨ NEW!");
...
console.log("✅ COMPLETE server configured with ALL routes including INVOICES");
...
console.log("✅ Server started successfully with QUOTES + INVOICES support!");
